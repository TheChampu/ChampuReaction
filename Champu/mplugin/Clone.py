...
from Champu import CLONE_OWNERS
from Champu import Champu as app, save_clonebot_owner
from Champu import db as mongodb, Champu
...
async def clone_txt(client, message):
    if len(message.command) > 1:
        ...
        try:
            ai = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="Champu/mplugin"))
            await ai.start()
            bot = await ai.get_me()
            bot_id = bot.id
